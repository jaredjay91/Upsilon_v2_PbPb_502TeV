...
void draw_v2_pt(int whichUpsilon=1) {
  ...
  for (int i=0; i<numptbins; i++) {
    hsyspt->SetBinContent(i+1,0.3);//0.036 is the largest systematic uncertainty tabulated in the J/Psi analysis.
  }
  ...
  int collId = kAADATA;
  int cLow = 0;
  if(collId == kPPDATA) CMS_lumi(c2, 1 ,33);
  else if(collId == kAADATA && cLow < 60) CMS_lumi(c2, 2 ,33);
