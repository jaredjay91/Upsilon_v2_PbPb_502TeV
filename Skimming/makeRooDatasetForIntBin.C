...
void makeRooDatasetForIntBin(int dateStr=20210901, bool NomAccTrue=kTRUE, bool NomEffTrue=kTRUE) 
{
  ...
  TFile* inFile = TFile::Open("condor/skims/newOniaTree_Skim_UpsTrig_RD_RAW_n-1_20210823_FullCent.root","READ");
  ...
  argSet->add(*cBinVar); argSet->add(*recoQQsign); argSet->add(*evtWeight);
  ...
  newfile = new TFile(Form("skims/newOniaTree_Skim_UpsTrig_RD_RAW_n-1_FullCent_withDataset_%s%s%i.root", accstr.Data(), effstr.Data(), dateStr),"recreate");
