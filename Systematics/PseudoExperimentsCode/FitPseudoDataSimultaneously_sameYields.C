...
void FitPseudoDataSimultaneously_sameYields( 
       int collId = kAADATA,  
			...
			) 
{
  ...
  for (int i=0; i<4; i++) {
    ...
    if (nSig3s_init[i] < 0) nSig3s_init[i] = 0;
  }
  ...
  TString bkgStr = "bkg";
  if (ptLow >= 5) bkgStr = "bkgHighPt";
  else bkgStr = "bkgLowPt";
  ...
  RooDataSet* signal1DS[4];
  RooDataSet* signal2DS[4];
  RooDataSet* signal3DS[4];
  //RooDataSet* backgroundDS[4];
  RooDataSet* reducedDS[4];
  for (int i=0; i<4; i++) {
    signal1DS[i] = (RooDataSet*)((RooAbsPdf*)genModel[i]->getComponents()->find("cb1s"))->generate(*(wsgen->var("mass")), nSig1s_init[i]);
    signal2DS[i] = (RooDataSet*)((RooAbsPdf*)genModel[i]->getComponents()->find("cb2s"))->generate(*(wsgen->var("mass")), nSig2s_init[i]);
    signal3DS[i] = (RooDataSet*)((RooAbsPdf*)genModel[i]->getComponents()->find("cb2s"))->generate(*(wsgen->var("mass")), nSig3s_init[i]);
    reducedDS[i] = (RooDataSet*)((RooAbsPdf*)genModel[i]->getComponents()->find(bkgStr))->generate(*(wsgen->var("mass")), nBkg_init[i]);
    cout << nSig1s_init[i] << " = " << signal1DS[i]->sumEntries() << endl;
    cout << nSig2s_init[i] << " = " << signal2DS[i]->sumEntries() << endl;
    cout << nSig3s_init[i] << " = " << signal3DS[i]->sumEntries() << endl;
    cout << nBkg_init[i] << " = " << reducedDS[i]->sumEntries() << endl;
    //reducedDS[i] = genModel[i]->generate(*(wsgen->var("mass")));
    //reducedDS[i] = (RooDataSet*)signal1DS[i]->Clone();
    reducedDS[i]->append(*signal1DS[i]);
    reducedDS[i]->append(*signal2DS[i]);
    reducedDS[i]->append(*signal3DS[i]);
    //reducedDS[i]->append(backgroundDS[i]);
