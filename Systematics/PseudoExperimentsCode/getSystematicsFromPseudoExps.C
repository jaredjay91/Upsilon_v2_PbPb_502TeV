...
void getSystOneBin(TH1D* hSyst, int whichBin=0, float ptLow=0, float ptHigh=50, float yLow=0.0, float yHigh=2.4, int cLow=10, int cHigh=90, TString systStr="", int whichUpsilon=1) {
  ...
  cSyst->SaveAs(Form("/home/jared/Documents/Ubuntu_Overflow/Upsilon_v2_502TeV_ThesisNew/Systematics/PseudoExperimentsCode/Plots/%sPseudoExpResults_%s%s.pdf", systStr.Data(), whichUpsString.Data(), kineLabel.Data()));
...
void getSystematicsFromPseudoExps(int whichUpsilon=1, int whichSyst=5) {
  ...
  for (int i=0; i<numptbins; i++) {
    ptLow = ptbins[i];
    ptHigh = ptbins[i+1];
    getSystOneBin(hSystpt, i+1, ptLow, ptHigh, yLow, yHigh, cLow, cHigh, systStr, whichUpsilon);
  }
  ...
  for (int i=0; i<numybins; i++) {
    yLow = ybins[i];
    yHigh = ybins[i+1];
    getSystOneBin(hSysty, i+1, ptLow, ptHigh, yLow, yHigh, cLow, cHigh, systStr, whichUpsilon);
  }
  ...
  for (int i=0; i<numcbins; i++) {
    cLow = cbins[i];
    cHigh = cbins[i+1];
    getSystOneBin(hSystc, i+1, ptLow, ptHigh, yLow, yHigh, cLow, cHigh, systStr, whichUpsilon);
  }
